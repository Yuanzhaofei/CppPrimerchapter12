...
private:
	StrBlob file;//保存输入文件，与输出结果类共享，因为输出结果需要打印单词所在行
...
public:
	using line_no = vector<string>::size_type;//行数类型
	QueryResult(string s, shared_ptr<set<line_no>> l, StrBlob f) :sought(s), lines(l),file(f){}
